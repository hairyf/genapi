...
Preset configuration that defines how APIs are generated. Available presets:

**HTTP clients (.ts / .js):**

- `axios.ts` / `axios.js` - axios
- `fetch.ts` / `fetch.js` - native fetch
- `ky.ts` / `ky.js` - ky
- `got.ts` / `got.js` - got
- `ofetch.ts` / `ofetch.js` - ofetch
- `uni.ts` / `uni.js` - UniApp network library

**Data layer (TypeScript only):**

- `tanstackQuery.react` - @tanstack/react-query (fetcher + useQuery/useMutation)
- `tanstackQuery.vue` - @tanstack/vue-query
- `colada` or `tanstackQuery.colada` - @pinia/colada (Vue + Pinia)
